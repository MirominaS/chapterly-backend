...
export const createBookController = async (req, res) => {
  try {
    ...
    if (duplicate) {
      return res
        .status(400)
        .json({ message: "Book already exist in your library" });
    }
...
export const updateController = async (req, res) => {
  try {
    ...
    //get existing book
    const existingBook = await getBookByIdService(req.params.id, req.user.id);
    ...
    if (updatedData.current_page === 0) {
      ...
      updatedData.current_page === updatedData.total_pages &&
      updatedData.total_pages > 0
    ) {
      ...
      updatedData.status !== "Paused" &&
      updatedData.status !== "Dropped"
    ) {
    ...
    const progress =
      updatedBook.total_pages > 0
        ? Math.round((updatedBook.current_page / updatedBook.total_pages) * 100)
        : 0;
...
export const deleteBookController = async (req, res) => {
  try {
    ...
    const deleted = await deleteBookService(req.params.id, req.user.id);
    if (!deleted) {
      return res.status(404).json({ message: "Book not found" });
    }
